...
function update(to: number) {
  current.value = (to + slides.value.length) % slides.value.length
}

function next() { update(current.value + 1) }
function prev() { update(current.value - 1) }
function go(i: number) { update(i) }
...
<template>
  <section class="bg-black min-h-screen flex items-center justify-center overflow-hidden p-4 sm:p-8 relative">
    <!-- Background effects -->
    ...
    <div class="w-full max-w-6xl mx-auto">
      <div class="carousel-container relative" @mouseenter="stop" @mouseleave="start">
        <!-- Progress bar -->
        <div class="absolute top-0 left-0 right-0 h-1 bg-white/10 rounded-full overflow-hidden z-20">
          <div
            class="progress-bar absolute top-0 left-0 h-full bg-gradient-to-r from-violet-500 to-fuchsia-500"
          ...
          >
            <div class="w-full h-full p-4 sm:p-8">
              <div class="w-full h-full rounded-xl sm:rounded-2xl overflow-hidden relative group">
                ...
                <div class="absolute inset-0 bg-gradient-to-br mix-blend-overlay" :class="s.overlay"></div>
...
.carousel-track { transform-style: preserve-3d; transition: transform 0.5s cubic-bezier(0.23, 1, 0.32, 1); }

.carousel-item { backface-visibility: hidden; transition: all 0.5s cubic-bezier(0.23, 1, 0.32, 1); }
...
.nav-button { transition: all 0.3s; background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
@media (hover: hover) { .nav-button:hover { background: rgba(255, 255, 255, 0.2); transform: scale(1.1); } }
.nav-button:active { transform: scale(0.95); }

.progress-bar { transition: width 0.5s cubic-bezier(0.23, 1, 0.32, 1); }
</style>
